...
const Profile = ({ isScrolled = false }: ProfileProps) => {
  ...
  return (
    <MenuContext.Provider value={{ hideNavbar }}>
      <div
        className={clsx(
          'z-20 shadow-sm sm:shadow-none lg:border-none dark:border-b dark:border-neutral-800 bg-light dark:bg-dark lg:!bg-transparent w-full p-5 lg:relative lg:p-0',
          expandMenu && 'pb-0'
        )}
      >
        <div className='flex items-start justify-between lg:flex-col lg:space-y-4 md:px-2'>
          ...
          {isMobile && (
            <div
              className={clsx(
                'flex lg:hidden items-center gap-4 mt-2',
                expandMenu &&
